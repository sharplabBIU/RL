...
agents = [{'q_values': np.full(n_bandits, initial_q), 'alpha': alpha} for alpha in learning_rates]

# Add the 4th agent with dynamic learning rate
dynamic_agent = {'q_values': np.full(n_bandits, initial_q), 'alpha': np.ones(n_bandits), 'counts': np.zeros(n_bandits)}

# Store the rewards and choices
rewards = np.zeros((len(agents) + 1, n_episodes, n_trials), dtype=int)
choices = np.zeros((len(agents) + 1, n_episodes, n_trials), dtype=int)

#initialize random walk step size
stepsize=0.05

# Define the random walk function for bandit probabilities
def random_walk(probs, step_size=stepsize):
    new_probs = probs + np.random.normal(0, step_size, size=probs.shape)
    return np.clip(new_probs, 0.000001, 0.99999)

# Initialize the bandit probabilities
bandit_probs = np.array([0.8, 0.5, 0.2])
latent_probs=np.zeros((len(bandit_probs), n_episodes, n_trials), dtype=int)

# Run the episodes
for episode in range(n_episodes):
    # Reset the Q-values for each episode
    for agent in agents:
        agent['q_values'] = np.full(n_bandits, initial_q)
    dynamic_agent['q_values'] = np.full(n_bandits, initial_q)
    dynamic_agent['alpha'] = np.ones(n_bandits)
    dynamic_agent['counts'] = np.zeros(n_bandits)
    
    for t in range(n_trials):
        # Update the bandit probabilities with a random walk
        bandit_probs = random_walk(bandit_probs)
        print('bandit probs:{}, trial {}'.format(bandit_probs,t))
        ...
        bandit_probs = bandit_probs.reshape(3,)
        # print(np.shape(bandit_probs))
        latent_probs[:,episode,t]=bandit_probs
...
for i in range(3):
    axs[2].plot(np.mean(latent_probs,axis=1), label=f'Bandit {i+1}')
